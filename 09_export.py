...
from typing import Annotated, Literal, Union, Any, Self, Optional
...
class Yolo:
    ...
    def export(
            self,
            fmt: Annotated[
                Union[str, Literal["coreml", "engine", "openvino", "onnx", "tflite", "torchscript"]],
                Field(description="The format to export the model to")
            ] = "onnx",
            *,
            img_size: Annotated[int, Field(ge=128, le=1024)] = 640,
            is_half: bool = False,
        ...
        :param fmt: The format to export the model to.
            - "coreml": Apple-exclusive format, optimised for iOS and macOS
            - "engine": NVIDIA GPU-exclusive format, optimised for TensorRT
            - "openvino": Intel hardware-exclusive format, optimised for OpenVINO
            - "onnx": Cross-platform universal format, compatible with various inference engines
            - "tflite": Mobile-exclusive format, optimised for Android and edge devices
            - "torchscript": PyTorch-exclusive format, optimised for C++ production deployment
        :param img_size: The size of the input image.
        :param is_half: Whether to use half precision for the export.
        :param is_dynamic: Whether to allow dynamic input shapes.
        :return: None
        """
        self._model.export(
            format=fmt,
        ...
        if self._display:
            print(
                f"Exporting model to {fmt!r} format "
                f"with image size {img_size}, {'half' if is_half else 'full'} precision "
                f"and {'dynamic' if is_dynamic else 'static'} input shapes."
            )
...
def main() -> None:
    """ Main Function """
    solar_model = load_pretrained_model("runs/detect/train-20/weights/best.pt")
    ...
    with Yolo(solar_model, image, accelerator=detect_device(), display=True) as yolo:
        yolo.export(fmt="torchscript")
